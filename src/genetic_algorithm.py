...
class GeneticAlgorithm(GlobalOptimizer):
    ...
    def iteration(self) -> None:
        ...
        if self.parallel:
            ...
            for _ in range(self.num_clusters):
                ...
                clus = self.utility.generate_cluster(pos)  # type: ignore
                self.cluster_list.append(clus)
        ...
        for child in children:  # Add children to current generation
            clus = self.utility.generate_cluster(child)  # type: ignore
    ...
    def selection(self) -> List[Atoms]:
        ...
        self.configs.append(self.best_config)  # type: ignore
        if self.preserve:
            self.cluster_list = [
    ...
    def crossover(self, cluster1: Atoms, cluster2: Atoms) -> List[Atom]:
        ...
        """
        # Align clusters
        self.utility.align_cluster(cluster1)  # type: ignore
        self.utility.align_cluster(cluster2)  # type: ignore
        ...
        while True:
            ...
            # Split the clusters
            group11, group12, _ = self.utility.split_cluster(cluster1, p1, p2, p3)  # type: ignore
            group21, group22, _ = self.utility.split_cluster(cluster2, p1, p2, p3)  # type: ignore
            ...
            if (
                self.utility.configuration_validity(np.array(group1))  # type: ignore
                and len(group1) == self.num_atoms
            ):
                return child_1
            if (
                self.utility.configuration_validity(np.array(group2))  # type: ignore
        ...
        for i in self.mutation_probability:
            if i == "twist":
                if np.random.rand() <= self.mutation_probability[i]:
                    ...
                    self.utility.twist(cluster)  # type: ignore
            elif i == "angular":
                if np.random.rand() <= self.mutation_probability[i]:
                    print("Angular", flush=True)
                    self.utility.angular_movement(cluster)  # type: ignore
            elif i == "etching":
                ...
                if etching <= self.mutation_probability[i]:
                    if etching < self.mutation_probability[i] / 2:
                        ...
                        self.utility.etching_subtraction(  # type: ignore
                            cluster
                        )  # Perform etching mutation (-)
                    else:
                        print("Etching (+)", flush=True)
                        self.utility.etching_addition(  # type: ignore
                            cluster
                        )  # Perform etching mutation (+)
